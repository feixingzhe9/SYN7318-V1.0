...
#define SHORT_SIZE    (35-12) //约定短帧最短长度为小于23bytes
...
/*******************************************************************************
函 数 名:  Si4432_ChannelInit
功能说明:  初始化SI4432信道，包括MCU管脚设置，将SI4432通信速率设置为9600bps
参    数:  void
返 回 值:  无
*******************************************************************************/
static void Si4432_ChannelInit(void)
{
#if (Si4432_CHANNEL	== CHANNEL_1M)
	...
	#else
		SPI_RW_Reg(W_REG | 0x75, Channel_Get());  //频率设置 434
	#endif
...
static u8 Sign433Data_Process(u8 *data , u8 len)
{
	...
	pData =(UnitPara_t*)&pMsg->para.unit; //增加通讯结构体解析
					
	if(len <= 50)//正在测信号时不处理50bytes以上的数据
	{
		if(1 == Get_Encrypt433Flag())
		{
...
//			Fault_Upload(FAULT_2, SI4432_CRC_ERR, NULL); 2014.11.13 Unarty 屏蔽，此异常记录已失去意义	
		...
		{							
			...
			for ( ; seat < resVal1; seat++)		//放入433缓存		
			{
				data[seat%50] = SPI_RW_Reg(R_REG|0x7F, 0x00);
			}
			if(resVal1 >= SHORT_SIZE) //常规帧
			{	
				if(TestRecFlag)//中断时接收RSSI信号值保存，2015.6.11新的方法
				{
...
#if (COMMUNICATION_SHORT_FRAME == 1) //启用短帧			
			else  //短帧直接放入short_fifo    
			{
