...
#define UNIT_GROUP_NUM				320//(80)									//每个单元组的总数
...
/*单元场景数据EEPROM空间分配*/
#define SCENE_START_ADD				(0x7D0)//(0x400)									//场景起始地址
#define SCENE_SPACE					(0x1800)								//场景占用总空间(6K,最多只能配置255个场景，每级区域下只能配置16个场景）
...
#define AREA_SCENEFLAG_NUM			(64)//(16)									//每个区域场景标识总数
...
#define SCENE_INDEXEX_SPACE					(4)

#define SCENESAVEFLAG_START_ADD		(SCENESAVE_START_ADD - 0x20)			//设备场景数据存储标识  //Modify by jay  2016.01.25
#define SCENE_MAX_NUM				(SCENE_SPACE1_NUM + SCENE_SPACE2_NUM + SCENE_SPACE3_NUM)//(256)									//最大场景数
...
//场景延时参数EEPROM内存分配
#define SCENCETIMFLAG_START_ADD     (0x1C00)								//场景延时状态存储标记,记录时间存储块的标号
#define SCENETIMUSEFLAG_START_ADD   (SCENCETIMFLAG_START_ADD + 0x100)		//场景内存块使用标记EEPROM首地址
#define SCENETIMSAVE_START_ADD      (SCENETIMUSEFLAG_START_ADD + 0x20)		//场景时间数据存储首地址
//注意延时时间内存块的标号从1开始,最多支持32个
#define SCENE_TIMDATA_SPACE         (50)									//场景时间空间最大值
#define SCENE_TIMSPACE_NUM			(32)									//场景延时空间的总数

#define ALINK_START_ADDR            (0x2400)								//最简联动首地址
#define ALINK_SPACE                 (0x0700)                                //最简联动空间

#define ALINK_TIME_START_ADDR       (0x2B00)								//延时时间首地址
#define ALINK_TIME_START            (0x0200)								//延时时间空间
