...
MsgResult_t Set_IAPReady_Attr(UnitPara_t *pData)
{
	...
	if (Updata_Check(SOFT->version, (u32*)&pData->data[2], (u32*)&pData->data[6]))	//升级条件核对
	{
		...
		if (Updata_Ready(pData->data[12], &iapFlag))	//升级准备完成
		{
			...
			*(u16*)&pData->data[7] = *(u16*)USER_FLASH_AREA;
...
返 回 值：  消息执行结果
*******************************************************************************/
MsgResult_t Set_433Encrypt_Attr(UnitPara_t *pData)
{
	if ((pData->len != 1))	//参数区长度不合法
	{
		return PARA_LEN_ERR;
	}
	else if (pData->data[0] > 1)    //参数区内容不合法		
	{
		return PARA_MEANING_ERR;
	}
	
	if (pData->data[0]  != Get_433EncryptFlag())
	{
		if (Updata_433EncryptFlag(pData->data[0]) == FALSE)	//修改加密标记
		{		
			return CMD_EXE_ERR;
		}
		Thread_Login(ONCEDELAY, 0, 500, &WDG_FeedOFF);	//关闭喂狗，设备自动进入重启
	}
...
返 回 值:  	消息执行结果
*******************************************************************************/
MsgResult_t Get_433Encrypt_Attr(UnitPara_t *pData, u8 *rlen, u8 *rpara)
{
	*rpara = Get_433EncryptFlag();		//加密标记值
	*rlen = 1;
