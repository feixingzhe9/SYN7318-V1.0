...
typedef u32 (*Cmd_Put_PTR)(FIFO_t *, u8 *, u32 );
/*******************************************************************************
函 数 名：	SaveSerialNum
功能说明： 	消息反馈
参	  数： 	无
...
函 数 名：	Msg_IntoFifo
功能说明： 	将433消息放入队列
参	  数： 	aimFifo : 队列指针    pMsg:消息内容
返 回 值：	无
*******************************************************************************/
static void Msg_IntoFifo(FIFO_t *aimFifo , Communication_t *pMsg)
{
	u8 fifoErr = 0 ;
	Cmd_Put_PTR pCmd_Put = NULL;
	
	if (1 == Get_Encrypt433Flag())
	{
		pCmd_Put = EncryptCmd_put;
	}
	else
	{
		pCmd_Put = cmd_put;
	}
	if(FALSE == pCmd_Put(aimFifo, (u8*)pMsg, sizeof(Communication_t) + pMsg->para.len))
		fifoErr = 1 ;	
//队列溢出 上报提示
	if(fifoErr && aimFifo == &send433fifo)
		Fault_Upload(FAULT_2 , SEND_433_FIFO_OVER , NULL);//暂时不做处理
	else if(fifoErr && aimFifo == &rcv433fifo)
...
void Msg_Feedback(Communication_t *pMsg)
{
	...
	{
		Msg_IntoFifo(&rcv433fifo,pMsg);
	}
	if(pMsg->aim.object.id != gSysData.deviceID)//目标ID不是自己
	{
		Msg_IntoFifo(&send433fifo,pMsg);
	}		
}
