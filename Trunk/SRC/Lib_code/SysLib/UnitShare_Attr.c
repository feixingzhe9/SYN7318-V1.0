...
/*******************************************************************************
函 数 名:  	Set_ManyUnitEx_Attr
功能说明:  	设置多个单元属性,用带有时间属性的多属性执行
参    数:  	data[0]: 目标单元
			data[1]: 参数长度
返 回 值:  	消息执行结果
*******************************************************************************/
...
/*******************************************************************************
函 数 名:  	u8 Get_SingleUnit_AttrLen(u8 unit,u8 cmd)
功能说明:  	获取单个单元返回值的长度
参    数:  	unit:	操作单元
			cmd:	操作指令
返 回 值:  	返回Get属性接口返回值的长度(0表示返回失败)
*******************************************************************************/
u8 Get_SingleUnit_AttrLen(u8 unit,u8 cmd)
{
	u8 data[200],ExeAttrNum = 0u; 		//参数区	
	...
	do {
			...
			{
				if ((pAttrExe->pGet_Attr != NULL)
					&& (COMPLETE == pAttrExe->pGet_Attr((UnitPara_t*)data, &data[3], &data[4]))	//如果获取属性内容完成
					)
				{
					return (data[3]);
				}
